...
namespace IDKin.IM.Windows.View.EmailAlert
{
    ...
    public partial class EntCustomEmailItem : TabItem//, IComponentConnector
	{
		...
		private void InitialEmail()
		{
			if (DataModel.Instance.EmailList != null && DataModel.Instance.EmailList.Count > 0)
			{
				...
				{
					if (DataModel.Instance.EmailList[i].EMailType == EmailType.EnterperseCustomEmail)
					{
		...
		private void InitialModel(string mailId)
		{
			...
			this.modal.EMailType = EmailType.EnterperseCustomEmail;
        ...
        //        Uri resourceLocater = new Uri("/IDKin.IM.Windows;component/view/emailalert/entcustomemailitem.xaml", UriKind.Relative);
